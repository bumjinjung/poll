...
규칙:
- 질문은 3~80자, 가볍고 재미있게. 유머 OK.
- 선택지 라벨은 1~20자, 중복/의미중복 금지.
- 난이도: "선택이 팽팽하도록" 조절.
- 이모지는 반드시 1개씩, 적절한 이모지 사용.
- JSON 형식으로만 응답하세요. 다른 텍스트는 포함하지 마세요.`;

    const userPrompt = `다음 예시 느낌을 참고해 하나만 만들어주세요.
예시: 
- "민초 vs 반민초" (🍃 vs ❌)
- "탕수육 부먹 vs 찍먹" (🥄 vs 🥢)
- "아침형 인간 vs 밤형 인간" (🌅 vs 🌙)
출력은 스키마에 맞춰 생성하세요. 한국어로.

JSON 형식으로만 응답:
{"question":"질문","left":{"label":"선택지1","emoji":"이모지"},"right":{"label":"선택지2","emoji":"이모지"}}`;
...
function getFallbackTemplate(): PollTemplate {
  const fallbackTemplates = [
    {
      question: "치킨 vs 피자 중 뭐 드실래요?",
      left: { label: "치킨", emoji: "🍗" },
      right: { label: "피자", emoji: "🍕" }
    },
    {
      question: "민초 vs 반민초 중 어느 쪽이세요?",
      left: { label: "민초", emoji: "🍃" },
      right: { label: "반민초", emoji: "❌" }
    },
    {
      question: "부먹 vs 찍먹 중 어느 쪽이세요?",
      left: { label: "부먹", emoji: "🥄" },
      right: { label: "찍먹", emoji: "🥢" }
    },
    {
      question: "집에서 vs 밖에서 어디서 시간을 보내시나요?",
      left: { label: "집에서", emoji: "🏠" },
      right: { label: "밖에서", emoji: "🌃" }
    },
    {
      question: "운동 vs 휴식 중 뭐 하실래요?",
      left: { label: "운동", emoji: "💪" },
