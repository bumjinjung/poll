...
│   ├── api/
│   │   ├── admin/              # 관리자 API
│   │   │   ├── auto-generate/  # AI 설문 자동 생성
│   │   │   ├── daily-rotation/ # 일별 로테이션
│   │   │   ├── fix-history/    # 히스토리 수정
│   │   │   ├── manual-history/ # 히스토리 수동 추가
│   │   │   └── today/          # 오늘/내일 설문 설정
│   │   ├── vote/               # 투표 API
│   │   └── history/            # 히스토리 조회 API
│   ├── admin/                  # 관리자 페이지
│   ├── history/                # 히스토리 페이지
│   │   ├── Header.tsx          # 히스토리 헤더
│   │   ├── HistoryClient.tsx   # 히스토리 클라이언트
│   │   ├── HistoryItem.tsx     # 히스토리 아이템
│   │   ├── HistoryListSSR.tsx  # 히스토리 SSR
│   │   └── page.tsx            # 히스토리 페이지
│   ├── components/
...
### 4. 코드 정리 및 최적화 (2025-10-27)
**정리 사항**:
- 사용되지 않는 CSS 애니메이션 제거 (fadeIn, pulse, shine 등)
- 불필요한 상태 변수 제거 (synced, isUpdating)
- Next.js 기본 템플릿 파일들 제거 (public/*.svg)
- 디버그용 API 파일 제거 (debug/kv-test)
- 중복 문서 파일 정리 (GUIDE.md, SETUP_COMPLETE.md)
- 프로젝트 구조 최적화로 빌드 크기 감소
